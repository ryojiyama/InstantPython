...
converted_pdf/split_chapters/ 内の .txt ファイルを読み込み、1文ごとに分断された段落を
本来のまとまりに結合して出力ディレクトリに出力する。

使い方:
    python3 merge_paragraphs.py
    python3 merge_paragraphs.py --src converted_pdf/split_chapters --out converted_pdf/merged_chapters
...
def main():
    ...
    # .txt ファイルを取得 (必要に応じて .md も含める場合は両方取得可能)
    files = sorted(src_dir.glob("*.txt"))
    if not files:
        sys.exit("%s に .txt ファイルがありません" % src_dir)
